...
When a desktop problem does not crash anything, choose _Trigger > Investigate Problem_ in the Omarchy Menu (`Super + Space`) or run `omarchy agent investigate`. This opens the default coding agent with the `diagnose-omarchy` skill. It asks for the symptom and expected behavior, collects relevant local state, and checks whether the problem belongs to the application, Hyprland, Omarchy, or local configuration. If it finds a coredump, it uses the `diagnose-crash` skill. It does not upload diagnostics or post an issue without review.
